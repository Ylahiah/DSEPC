...
for /f "usebackq delims=" %%I in (`powershell -NoProfile -Command "(Get-NetIPAddress -AddressFamily IPv4 | Where-Object InterfaceAlias -Match 'Wi-Fi|Ethernet' | Where-Object InterfaceAlias -NotMatch 'Virtual|Bluetooth').IPAddress[0]" 2^>nul`) do set "HOST_IP=%%I"
